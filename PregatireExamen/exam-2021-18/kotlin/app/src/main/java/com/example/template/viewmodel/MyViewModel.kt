...
class MyViewModel(application: Application) : AndroidViewModel(application) {
    ...
    var data: LiveData<List<Exam>>? = null
    var changedData: LiveData<List<Exam>>? = null
    ...
    fun insertAll(gr: List<Exam>) = viewModelScope.launch(Dispatchers.IO) {
    ...
    fun insert(gr: Exam) = viewModelScope.launch(Dispatchers.IO) {
        logd("insert in  view model: $gr")
        repository.insert(gr)
    }

    fun update(gr: Exam) = viewModelScope.launch(Dispatchers.IO) {

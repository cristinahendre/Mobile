...
class LevelActivity : AppCompatActivity() {
    ...
    private fun fetchLevelData() {
        ...
        GlobalScope.launch(Dispatchers.Main) {
            ...
            try {
                val levelText = level.text.toString().toInt()

                val myData = model.getAllLevel(levelText)
                if (myData == null) {
                    displayMessage("The server is down.")
                } else {
                    val myList = myData.sortedBy { it.from; it.to; it.status }
                    displayData(myList)
                }
            } catch (e: NumberFormatException) {
                displayMessage("Choose a valid level")
            }
            progress.dismiss()
        }

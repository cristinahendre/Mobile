...
object RestService {
    ...
    interface Service {
        ...
        @GET("level/{level}")
        suspend fun getAllLevel(@Path("level") level:Int): List<Rule>

        @GET("rule/{id}")
        suspend fun findOne(@Path("id") id:Int):Rule

        @DELETE("{id}")
        suspend fun delete(@Path("id") id: Int): Response<Rule>

        @POST("rule")
        suspend fun add(
            @Body grade: RuleCredentials): Response<Rule>

        @POST("update")
        suspend fun update(
            @Body grade: RuleCredentials
        ): Response<Rule>

...
class MyViewModel(application: Application) : AndroidViewModel(application) {
            ...
            try{
                WebSocketStuff.start() {
                    viewModelScope.launch {
                        logd("aici $it")
                        val text = "Studentul ${it.nume} are media1 ${it.medie1}," +
                                " media2 ${it.medie2}; dosarul are status ${it.status}"
                        Toast.makeText(
    ...
    fun getNevalidate() {
        logd("get nevalidate in view model")
        data = repository.getNevalidate()
    }
